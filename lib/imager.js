...
var Imager = module.exports = function Imager(config, storage, bucket) {
    ...
    if ((bucket === undefined) && (storage === 'Local') && (config.storage['Local'].path === undefined)) {
        throw new Error('Please specify buckets for local storage OR define path for local storage');
    } //else if (config.storage.Local.this[bucket].path === undefined) {
    //  throw new Error('Please specify path attribute for the bucket.');
    // }
...
Imager.prototype = {
    ...
    upload: function(files, callback, variant) {
        ...
        if (typeof variant !== 'string' && !this.config.variants.
            default) {
            throw new Error('Please specify a proper variant OR provide a default');
        }

        if (!variant && this.config.variants.
            default) {
            variant = 'default';
        }
    ...
    pushToLocal: function(tempFile, localFile, filename, type, cb) {
        ...
        // If bucket is undefined, use storage.local.path instead of bucket path
        if (this.bucket === undefined) {
            var bucketPath = this.config['storage']['Local'].path;
            var mode = this.config['storage']['Local'].mode || 0777;
        } else {
            var bucketPath = this.config['storage']['Local'][this.bucket].path;
            var mode = this.config['storage']['Local'][this.bucket].mode || 0777;
        }

        var localPath = path.resolve(path.join( // find destination path
            bucketPath, directory, localFile
        ));
    ...
    getClientForRackspace: function(cb) {
        ...
        if (!container) {
            client.containerCache[clientConfig.container] = {
                connecting: true
            };
    ...
    pushToS3: function(tempFile, remoteFile, filename, type, cb) {
        ...
        client.putFile(tempFile, directory + remoteFile, options, function(err, res) {
            ...
            cdn = s3Config['cdn'];
            if (cdn) {
                self.cdnUri = (~cdn.indexOf('http://') || ~cdn.indexOf('https://')) ? cdn : 'http://' + cdn;
            } else {
